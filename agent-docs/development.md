# Development
Development for any features or tasks must be done after creating a plan. Prompt the user to create a plan.md document if there is not one already.

Make sure you are on main or master branch with latest changes pulled unless specified otherwise.
Checkout a new branch, and prompt user for branch name if not provided

Always follow Test-Driven Development (TDD) approach. 

First, come up with test cases following the Testing section, then run them and make sure they fail. 
Then, implement the functionality.
Verify the functionality works by running the tests again to make sure they pass.

Changes should be grouped into separate commits with clear message indicating what was changed in that commit. 
